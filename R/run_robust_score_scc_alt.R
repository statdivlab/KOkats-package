...
#' }
#' 
#' res <- run_robust_score_scc_alt(Y = Y, X = X, ncores = 2, null_k = 2, null_j = 2)
#' 
#' @export
run_robust_score_scc_alt <- function(formula_rhs = NULL,
                                 Y,
                                 X = NULL,
                                 covariate_data = NULL,
                                 B = NULL,
  ...
  for (i in 1:n) {
      ...
      for (ind in 1:n) {
        if (ind == i) {
          score_first[ind + p*(J - 1)] <- Y[i, 1] - exp(z_mle[i]) + 
            Y[i, j] - sum(exp(X[i, ] %*% B_mle[, js] + z_mle[i]))
        }
      }
