#' Fit Algorithm 1
#' Estimate B and z parameters through block coordinate descent to maximize the log likelihood function, details given in Algorithm 1, using temporary first KO constraint through coordinate descent.
#'
...
#'
#' @return A list including values of the log likelihood, the B matrix, and the z vector at each iteration, and the final B estimate.
#'
#' @examples
...
fit_alg1_alt <- function(formula_rhs = NULL,
  ...
  if (is.null(B)) {
    ...
    # use temporary first KO coefficients = 0 constraint 
    for (k in 1:p) {
      B[k, ] = B[k, ] - B[k, 1]
    }
  }
