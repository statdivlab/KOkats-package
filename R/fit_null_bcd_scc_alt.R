...
#' }
#' 
#' res <- fit_null_bcd_scc_alt(Y = Y, X = X, ncores = 2, null_k = 2, null_j = 2)
#' 
#' @export
fit_null_bcd_scc_alt <- function(formula_rhs = NULL,
                             Y,
                             X = NULL,
                             covariate_data = NULL,
                             B = NULL,
  ...
  while (condition) {
    ...
    while (t_nr == 1 | (sum(((B_new - B_old)/B_old)^2) > tolerance_nr 
      ...
      for (r in 1:(p*(J - 1) - 1)) {
        F_B[r] <- sum(Y[, j_vec[r]]*X[, k_vec[r]] - 
                        X[, k_vec[r]]*exp(X %*% B_mat[, j_vec[r]] + z))
        for (c in 1:r) {
          val <- 0
          if (j_vec[r] == j_vec[c]) {
            val <- sum(-X[, k_vec[r]]*X[, k_vec[c]]*exp(X %*% B_mat[, j_vec[r]] + z))
          }
          J_B[r, c] <- val
          J_B[c, r] <- val
        }
  ...
  return(list(likelihood = lik_vec, B = B_array, z = z_array,
              ...
              score_mat = score_mat, converged = converged))
}
